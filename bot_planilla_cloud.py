# bot_planta_cloud.py - Bot Completo para la Nube
import os
import asyncio
import json
import re
...
# --- CONFIGURACIÓN CLOUD ---
TELEGRAM_BOT_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN', '7518117287:AAFqswPQ5N4powkuB5UzRm6oMfHG_UNGWrI')
...
JSON_DATA_FILE = 'registros_planta.json'
JSON_BACKUP_FILE = 'registros_backup.json'

# --- PATRONES DE VALIDACIÓN ---
VALIDATION_PATTERNS = {
    'fecha': re.compile(r'^\d{2}/\d{2}/\d{4}# bot_planta_cloud.py - Versión optimizada para la nube con almacenamiento JSON
import os
import asyncio
import json
import re
...
# --- CONFIGURACIÓN CLOUD ---
# Variables de entorno (se configuran en Render.com)
TELEGRAM_BOT_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN', '7518117287:AAFqswPQ5N4powkuB5UzRm6oMfHG_UNGWrI')
GEMINI_API_KEY = os.getenv('GEMINI_API_KEY', 'AIzaSyDj-6_v_nk6HsCC-e7AtXnwWgL54BSUUtQ')
...
FIELD_CONFIG = {
    ...
    'hora': {
        ...
        'quick_options': ['8:00', '10:00', '12:00', '14:00', '16:00', '18:00', '✅ Finalizar'],
        'validator': 'hora',
        'multiple': True
    },
    'visita': {
...
class CloudDataManager:
    ...
    def save_record(record_data: dict) -> bool:
        ...
        try:
            # Preparar registro con metadatos
            new_record = {
                'id': datetime.now().strftime('%Y%m%d_%H%M%S_%f')[:-3],  # ID único
    ...
    def get_stats() -> dict:
        ...
        return {
            ...
            'file_size_kb': round(os.path.getsize(JSON_DATA_FILE) / 1024, 2) if os.path.exists(JSON_DATA_FILE) else 0
        }
    
    @staticmethod
    def clear_records() -> bool:
        """Limpia todos los registros (para sincronización)"""
        try:
            # Mantener metadatos pero limpiar registros
            clean_data = {
                'metadata': {
...
# [INCLUIR TODAS LAS FUNCIONES DEL BOT ANTERIOR AQUÍ]
# Por brevedad, incluyo solo las funciones clave modificadas para cloud
    ...
    try:
        with open(JSON_DATA_FILE, 'rb') as f:
            await context.bot.send_document(
                ...
                document=f,
                filename=f"registros_planta_{datetime.now().strftime('%Y%m%d_%H%M')}.json",
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Maneja entrada de texto con confirmación de limpieza cloud"""
    
    # Verificar confirmación de limpieza cloud
    ...
    # [RESTO DE LA LÓGICA DE handle_text_input DEL BOT ANTERIOR]
...
def main():
    ...
    logger.info("🌱 Bot Cloud iniciando...")
    ...
    app.run_webhook(
        ...
        webhook_url=f"https://tu-app.onrender.com/{TELEGRAM_BOT_TOKEN}"
    )

if __name__ == '__main__':
    main()),
    'hora': re.compile(r'^\d{1,2}:\d{2}# bot_planta_cloud.py - Versión optimizada para la nube con almacenamiento JSON
import os
import asyncio
import json
import re
...
if __name__ == '__main__':
    main()),
    'numero': re.compile(r'^\d*\.?\d+# bot_planta_cloud.py - Versión optimizada para la nube con almacenamiento JSON
import os
import asyncio
import json
import re
...
if __name__ == '__main__':
    main()),
    'si_no': re.compile(r'^(SI|NO)# bot_planta_cloud.py - Versión optimizada para la nube con almacenamiento JSON
import os
import asyncio
import json
import re
...
if __name__ == '__main__':
    main(), re.IGNORECASE),
    'nivel': re.compile(r'^(Alto|Medio|Bajo)# bot_planta_cloud.py - Versión optimizada para la nube con almacenamiento JSON
import os
import asyncio
import json
import re
...
def main():
    ...
    # Handlers
    app.add_handler(CommandHandler("start# bot_planta_cloud.py - Versión optimizada para la nube con almacenamiento JSON
import os
import asyncio
import json
import re
